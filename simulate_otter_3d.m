% Simulation script for the 3-DOF Otter USV in a straight line
% --- Simulation Parameters ---
T_sim = 100;         % Total simulation time (s)
dt = 0.01;            % Time step (s)
time = 0:dt:T_sim;   % Time vector
% --- Otter USV Parameters ---
x_initial = zeros(6, 1); % Initial state: [x y yaw u v r]'
n_desired = [100; 100];    % Desired propeller shaft speeds (rad/s) - adjust for desired speed
...
plot(time, x_history(5, :));
xlabel('Time (s)');
ylabel('Speed (m/s)');
title('Speed');
grid on;
sgtitle('3-DOF Otter USV Straight Line Simulation');