%%  RSC (Random Sequential Composition) With Circular Funnels

close all;
rng(69); % seed

% Choose  and get scenario
scenarioId = 3;   % 1 or 2
[W, obs, q_start, q_goal] = getScenario(scenarioId);

%% RSC PARAMETERS

P.minCircArea   = 3.0; % minimum circle area (m^2)
P.maxRadius     = inf; % maximum circle radius (m)
P.safetyMargin  = 2.0; % safety margin to obstacles (m)
P.expandStep    = 0.1; % circle expansion step size (m)
P.eta           = 0.9; % d = eta * r_f
P.maxIter       = 500; % maximum iterations
P.circleRes     = 100; % # of vertices for circle resolution

%% BUILD RSC TREE

% node struct
nodes = struct('poly', {}, 'c', {}, 'id', {}, 'radius', {}, 'parent', {});

% worksapce (map)
workPoly = polyshape([W(1) W(2) W(2) W(1)], [W(3) W(3) W(4) W(4)]);

% Check if start and goal points are unoccupied
assert(isFreePoint(q_start, obs, workPoly), 'Start is in obstacle/outside workspace.');
assert(isFreePoint(q_goal,  obs, workPoly), 'Goal is in obstacle/outside workspace.');

% accepted nodes
accepted = 0;

% add goal point as master funnel 
[rootPoly, rootRadius, ~] = buildCircularNode(q_goal, obs, W, workPoly, P);

if isempty(rootPoly) || area(rootPoly) < P.minCircArea
    error('Master funnel at GOAL could not be generated.');
end

% increase the accepted node and fill the info
accepted = accepted + 1;
nodes(accepted).poly   = rootPoly;
nodes(accepted).radius = rootRadius;
nodes(accepted).c      = q_goal;
nodes(accepted).id     = accepted;
nodes(accepted).parent = 0;

goalId  = 1;     % root/master funnel id
startId = [];    % will be set when a funnel covers q_start

fprintf('Added GOAL master node as id=%d\n', goalId);

% Trivial case: start already inside master funnel
if isinterior(nodes(goalId).poly, q_start(1), q_start(2))
    startId = goalId;
    fprintf('Start is already inside the master funnel. Trivial one-funnel solution.\n');
end

iter = 0;
while iter < P.maxIter && isempty(startId)

    % Sample a free point in Cartesian space
    q = sampleFreePoint(W, obs, workPoly);
    if isempty(q)
        continue;
    end

    % Omit sample if already inside any existing funnel
    if isCovered(q, nodes)
        continue;
    end

    % Find closest existing funnel by boundary distance
    parentId = findNearestFunnel(q, nodes);
    qparent  = nodes(parentId).c;
    Rparent  = nodes(parentId).radius;

    % Project sample onto nearest funnel boundary direction
    qproj = projectPointToCircleBoundary(q, qparent, Rparent);

    dir = qproj - qparent;
    nd  = norm(dir);
    if nd < 1e-12
        continue;
    end
    dir = dir / nd;

    % Paper: qnew is inside nearest funnel, near boundary
    qnew = qparent + P.eta * Rparent * dir;

    if ~isFreePoint(qnew, obs, workPoly)
        continue;
    end

    % generate new circular funnel centered at qnew
    [nodePoly, radius, ~] = buildCircularNode(qnew, obs, W, workPoly, P);

    if isempty(nodePoly) || area(nodePoly) < P.minCircArea
        continue;
    end

    % increase the accepted node and fill the info
    accepted = accepted + 1;
    nodes(accepted).poly   = nodePoly;
    nodes(accepted).radius = radius;
    nodes(accepted).c      = qnew;
    nodes(accepted).id     = accepted;
    nodes(accepted).parent = parentId;

    fprintf('iter=%d | added node=%d | parent=%d | Rparent=%.3f | Rnew=%.3f\n', ...
        iter, accepted, parentId, Rparent, radius);

    % Stop when new funnel covers q_start
    if isinterior(nodePoly, q_start(1), q_start(2))
        startId = accepted;
        fprintf('Start reached at iter=%d with %d funnels.\n', iter, accepted);
        break;
    end

    iter = iter + 1;
end

fprintf('Terminated: accepted=%d, iter=%d (threshold=%d)\n', accepted, iter, P.maxIter);
fprintf('Accepted nodes: %d\n', accepted);

if isempty(startId)
    warning('Start was not reached within max iterations.');
end

%% EXTRACT NODE-PATH

% Collect 
pathIds = [];

if ~isempty(startId)
    cur = startId;
    while cur ~= 0
        pathIds(end+1) = cur;
        cur = nodes(cur).parent;
    end
end

if isempty(pathIds)
    warning('No path found in the tree.');
else
    fprintf('Found path with %d nodes.\n', numel(pathIds));
end

fprintf('startId=%d, goalId=%d\n', safeId(startId), safeId(goalId));

%% PLOTTING
% RSC tree figure
fig1 = figure('WindowState','maximized', 'Color','w'); 
ax1 = axes('Parent', fig1); hold(ax1, 'on'); axis(ax1, 'equal');
xlim(ax1, [W(1) W(2)]); ylim(ax1, [W(3) W(4)]);
set(ax1, 'XTick', [], 'YTick', [], 'Box', 'on');

% Remove White Margins for Figure 1
set(ax1, 'LooseInset', [0,0,0,0]);
ax1.Position = [0 0 1 1]; 

% Plot obstacles
for i=1:numel(obs)
    plot(ax1, obs{i}, 'FaceColor',[0 0 0], 'FaceAlpha',0.6, 'EdgeColor','none');
end

% Plot nodes
for i=1:numel(nodes)
    plot(ax1, nodes(i).poly, 'FaceColor',[0.7 0.85 1.0], 'FaceAlpha',0.10, ...
        'EdgeColor',[0.3 0.6 1.0], 'LineWidth',0.5);
    plot(ax1, nodes(i).c(1), nodes(i).c(2), '.', 'Color',[0.2 0.4 0.9], 'MarkerSize',10);
    if nodes(i).parent ~= 0
        pc = nodes(nodes(i).parent).c;
        cc = nodes(i).c;
        plot(ax1, [cc(1) pc(1)], [cc(2) pc(2)], '--', 'Color',[0.5 0.5 0.5], 'LineWidth',0.7);
    end
end

% Plot start and goal points
plot(ax1, q_start(1), q_start(2), 'go', 'MarkerSize',9, 'LineWidth',2);
text(ax1, q_start(1), q_start(2), '  start', 'FontSize', 12, 'FontWeight', 'bold');
plot(ax1, q_goal(1),  q_goal(2),  'ro', 'MarkerSize',9, 'LineWidth',2);
text(ax1, q_goal(1), q_goal(2), '  goal', 'FontSize', 12, 'FontWeight', 'bold');

% -------------------------------------------------------------------------
% Solution figure
fig2 = figure('WindowState','maximized', 'Color','w'); 
ax2 = axes('Parent', fig2); hold(ax2, 'on'); axis(ax2, 'equal');
xlim(ax2, [W(1) W(2)]); ylim(ax2, [W(3) W(4)]);
set(ax2, 'XTick', [], 'YTick', [], 'Box', 'on');

% Remove White Margins for Figure 2
set(ax2, 'LooseInset', [0,0,0,0]);
ax2.Position = [0 0 1 1];

% Plot obstacles
for i=1:numel(obs)
    plot(ax2, obs{i}, 'FaceColor',[0 0 0], 'FaceAlpha',0.6, 'EdgeColor','none');
end

if ~isempty(pathIds)
    for k = 1:length(pathIds)
        node_idx = pathIds(k);
        plot(ax2, nodes(node_idx).poly, 'FaceColor',[1.0 0.85 0.7], ...
            'FaceAlpha',0.40, 'EdgeColor',[1.0 0.5 0.0], 'LineWidth',1.5);
    end
    
    % Waypoints logic
    num_nodes = length(pathIds);
    route_points = zeros(num_nodes + 1, 2);
    route_points(1, :) = q_start;
    for k = 1:(num_nodes - 1)
        curr_node = nodes(pathIds(k)).poly;
        next_node = nodes(pathIds(k+1)).poly;
        intersection_poly = intersect(curr_node, next_node);
        if isempty(intersection_poly) || intersection_poly.NumRegions == 0 || area(intersection_poly) <= 0
            cp = 0.5 * (nodes(pathIds(k)).c + nodes(pathIds(k+1)).c);
        else
            [cx, cy] = centroid(intersection_poly);
            cp = [cx, cy];
        end
        route_points(k+1, :) = cp;
    end
    route_points(end, :) = q_goal;
    plot(ax2, route_points(:,1), route_points(:,2), 'bo-', 'MarkerSize',6, 'LineWidth',1.5);
end

% Plot start and goal points
plot(ax2, q_start(1), q_start(2), 'go', 'MarkerSize',9, 'LineWidth',2);
text(ax2, q_start(1), q_start(2), '  start', 'FontSize', 12, 'FontWeight', 'bold');
plot(ax2, q_goal(1),  q_goal(2),  'ro', 'MarkerSize',9, 'LineWidth',2);
text(ax2, q_goal(1), q_goal(2), '  goal', 'FontSize', 12, 'FontWeight', 'bold');

%% FUNCTIONS

% point = sampleFreePoint(map corner points, obstacles, map polygon)
function q = sampleFreePoint(W, obs, workPoly)
    for t = 1:200
        q = [W(1) + (W(2)-W(1))*rand, W(3) + (W(4)-W(3))*rand];
        if isFreePoint(q, obs, workPoly), return; end
    end
    q = [];
end

% ok = isFreePoint(point, obstacles, map polygon)
function ok = isFreePoint(q, obs, workPoly)
    if ~isinterior(workPoly, q(1), q(2)), ok = false; return; end % check if inside the map

    for i = 1:numel(obs)
        if isinterior(obs{i}, q(1), q(2)), ok = false; return; end % check if intersects with obstacles
    end

    ok = true;
end

% check if polygon inside the map
function inside = isPolyInsideWorkspace(Psh, workPoly)
    if isempty(Psh) || Psh.NumRegions == 0
        inside = false; return;
    end
    Pint = intersect(Psh, workPoly);
    inside = ~isempty(Pint) && Pint.NumRegions > 0 && abs(area(Pint) - area(Psh)) < 1e-9;
end

% create circular funnel
function poly = circularPoly(center, radius)
    numPoints = 100;
    theta = linspace(0, 2*pi, numPoints + 1)';
    theta(end) = [];   % remove duplicate endpoint

    x = center(1) + radius * cos(theta);
    y = center(2) + radius * sin(theta);

    poly = polyshape(x, y);
end

% check if polygon intersects with obstacle
function coll = circCollides(poly, obs, margin)
    coll = false;
    for i = 1:numel(obs)
        ob = obs{i};
        if margin > 0
            try, ob = polybuffer(ob, margin); catch, end
        end
        interP = intersect(poly, ob);
        if ~isempty(interP) && interP.NumRegions > 0 && area(interP) > 0
            coll = true; return;
        end
    end
end

% find closest point on the obstacle boundaries to the point qrand
function [qobs, dmin] = closestObstaclePoint(qrand, obs)
    dmin = inf;
    qobs = [];
    for i = 1:numel(obs)
        [vx, vy] = boundary(obs{i});
        if isempty(vx), continue; end
        V = [vx(:) vy(:)];
        pts = [];
        for k = 1:size(V,1)-1
            pA = V(k,:); pB = V(k+1,:);
            a = linspace(0,1,400)';
            pts = [pts; (1-a).*pA + a.*pB]; %#ok<AGROW>
        end
        D = pts - qrand;
        dd = sum(D.^2,2);
        [m, idx] = min(dd);
        if m < dmin^2
            dmin = sqrt(m);
            qobs = pts(idx,:);
        end
    end
end

% compute the distance from point q to the nearest workspace boundary
function dbox = distanceToBoxBoundary(q, W)
    dbox = min([q(1)-W(1), W(2)-q(1), q(2)-W(3), W(4)-q(2)]);
end

% builds one funnel node centered at qrand
function [poly, radius, qobs] = buildCircularNode(qrand, obs, W, workPoly, P)
    poly = [];
    radius = 0;
    qobs = [];

    [qobs, dminObs] = closestObstaclePoint(qrand, obs);
    dminBox = distanceToBoxBoundary(qrand, W);

    % Subtract safety margin immediately so the initial poly is valid
    radius = min([dminObs, dminBox, P.maxRadius]) - P.safetyMargin;

    % Basic validity checks
    if radius <= 0 || ~isfinite(radius), return; end

    center = qrand;
    poly = circularPoly(center, radius);

    % Check if initial circle is valid
    if ~isPolyInsideWorkspace(poly, workPoly) || circCollides(poly, obs, P.safetyMargin)
        poly = [];
        radius = 0;
        return;
    end

    % Expand from the valid base
    radius = expandNode(qrand, radius, obs, W, workPoly, P);
    poly = circularPoly(qrand, radius);
end

% enlarge the circle
function radius = expandNode(center, radius, obs, W, workPoly, P)
    while radius + P.expandStep <= P.maxRadius
        candR = radius + P.expandStep;

        if candR > distanceToBoxBoundary(center, W) - P.safetyMargin
            break;
        end

        cand = circularPoly(center, candR);
        if ~isPolyInsideWorkspace(cand, workPoly) || circCollides(cand, obs, P.safetyMargin)
            break;
        end

        radius = candR;
    end
end

% Check if the point is in any of the nodes
function tf = isCovered(q, nodes)
    tf = false;
    for i = 1:numel(nodes)
        if isinterior(nodes(i).poly, q(1), q(2))
            tf = true;
            return;
        end
    end
end

% Find the funnel whose boundary is closest to the point q
function parentId = findNearestFunnel(q, nodes)
    bestVal = inf;
    parentId = 1;

    for i = 1:numel(nodes)
        d = abs(norm(q - nodes(i).c) - nodes(i).radius);
        if d < bestVal
            bestVal = d;
            parentId = i;
        end
    end
end

% Projects point q onto the boundary of the circle centered at c with radius R
function qproj = projectPointToCircleBoundary(q, c, R)
    d = q - c;
    nd = norm(d);

    if nd < 1e-12
        qproj = c + [R 0];
    else
        qproj = c + R * d / nd;
    end
end

% If idval is empty, return -1; otherwise return the actual ID
function out = safeId(idval)
    if isempty(idval)
        out = -1;
    else
        out = idval;
    end
end